...
interface StepsListProps {
  ...
  readOnly?: boolean; // Disable editing when true (e.g., during re-analysis)
}
...
interface SubprocessItemProps {
  node: FlowNode;
  onSubprocessClick?: (id: string) => void;
  subprocesses?: any[];
}

function SubprocessItem({ node, onSubprocessClick, subprocesses }: SubprocessItemProps) {
    // Find information about the subprocess if available
    const subprocessInfo = subprocesses?.find(p => p._id === node.subprocessId || p.processName === node.subprocessId);

    const handleClick = () => {
      if (onSubprocessClick && node.subprocessId) {
        ...
        <div className="flex h-full">
          ...
          <div className="flex-1 p-3 min-w-0 flex flex-col">
            <div className="flex items-start gap-2">
              {/* Subprocess icon (similar to step number circle) */}
              <div className="flex-shrink-0 w-7 h-7 rounded-full bg-violet-600 text-white flex items-center justify-center">
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-violet-100 text-violet-700 border border-violet-200">
                        SUB-PROCESS
                      </span>
                      {subprocessInfo && (
                        <span className="text-xs text-muted-foreground">
                          {subprocessInfo.totalSteps || 0} steps
                        </span>
                      )}
                    </div>
                    <p className="text-sm font-medium leading-snug break-words group-hover:text-violet-700 transition-colors">
                      {subprocessInfo?.processName || node.label || "Sub-process"}
                    </p>
                    <p className="text-xs text-muted-foreground line-clamp-2">
                      {subprocessInfo?.processDescription || "Click to view sub-process details and steps."}
                    </p>
                  </div>
          ...
          <div className="w-24 flex-shrink-0 bg-gradient-to-br from-violet-50 to-violet-100/50 rounded-r-lg flex items-center justify-center border-l">
            <div className="text-center">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-8 h-8 text-violet-400 mx-auto mb-1">
              ...
              <span className="text-[10px] font-medium text-violet-500 uppercase tracking-wide">
                {subprocessInfo?.totalSteps || "?"} Steps
              </span>
            </div>
          </div>
        </div>
  ...
  function StepItem({ step, isExpanded, onToggle, onOpenLightbox, onEdit, onDelete, isDeleting, onAddAfter, readOnly }: StepItemProps) {
    ...
    return (
      <div id={`step-${step.stepNumber}`} className="border rounded-lg h-full relative overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {/* Edit button - hidden during re-analysis */}
                    {!readOnly && (
                      <Button
                        variant="ghost"
                        size="sm"
                    ...
                    {/* Delete button - hidden during re-analysis */}
                    {!readOnly && (
                      <Button
                        variant="ghost"
                        size="sm"
        ...
        {/* Add step after button - bottom left corner - hidden during re-analysis */}
        {!readOnly && (
          <Button
            variant="ghost"
            size="sm"
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  export function StepsList({ steps, processId, selectedStepNumber, onStepSelect, flowNodes, flowEdges, subprocesses, onSubprocessClick, readOnly }: StepsListProps) {
    ...
    const unifiedList = useMemo(() => {
      ...
      const traverseSequential = (nodeId: string, stopAtMerge: boolean = false): void => {
        ...
        if (node.nodeType === "action") {
          ...
          items.push({ type: "subprocess", node });
        }
      ...
      const collectBranchItems = (nodeId: string): BranchItem[] => {
        ...
        const collect = (nId: string): void => {
          ...
          if (node.nodeType === "action") {
            ...
            branchItems.push({ type: "subprocess", node });
          } else if (node.nodeType === "decision" || node.nodeType === "switch") {
            // Add the decision
      ...
      while (pendingBranches.length > 0) {
        ...
        branchItems.forEach(branchItem => {
          ...
          } else if (branchItem.type === "subprocess") {
            items.push({ type: "subprocess", node: branchItem.node });
      ...
      const missingSubprocesses = (subprocesses || [])
      ...
      return items;
    }, [steps, flowNodes, flowEdges, subprocesses, showDecisions]);
    ...
    <CardHeader>
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {processId && (
            <>
              {/* Add Step button - hidden during re-analysis */}
              {!readOnly && (
                <Button
                  variant="outline"
                  size="sm"
      ...
      <div className={`grid ${columnGridClasses[columns]}`}>
        {unifiedList.map((item, index) => {
          ...
          if (item.type === "subprocess") {
            return (
              <SubprocessItem
                key={`subprocess-${item.node.nodeId}`}
                node={item.node}
